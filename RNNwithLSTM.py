...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    hi = h0
    for t in range(Tx):
        hi, _ = rnn_step_forward(x[:, t, :], hi, Wx, Wh, b)
        h[:, t, :] = hi
        cache.append(_)
...
def rnn_backward(dh, cache):
    ...
    for l in reversed(range(T)):
        cur_dh = dh_prev + dh[:, l, :]
        res = rnn_step_backward(cur_dh, cache[l])
        dx[:, l, :] = res[0]
        dh_prev = res[1]
        dWx += res[2]
        dWh += res[3]
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    H = prev_h.shape[1]
    a = np.dot(x, Wx) + np.dot(prev_h, Wh) + b # candidate c
    ...
    next_c = forget_gate * prev_c + input_gate * c_tilda
    next_h = output_gate * np.tanh(next_c)

    cache = (x, Wx, Wh, b, prev_h, prev_c, input_gate, forget_gate, output_gate, c_tilda , next_c, next_h)
